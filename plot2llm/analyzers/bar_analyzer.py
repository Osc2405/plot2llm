...
def analyze(ax, x_type=None, y_type=None) -> Dict[str, Any]:
    ...
    max_category = stats.get("categorical_analysis", {}).get("most_frequent_category", "unknown") if 'stats' in locals() else "unknown"
    min_category = stats.get("categorical_analysis", {}).get("least_frequent_category", "unknown") if 'stats' in locals() else "unknown"
