def build_data_summary_section(semantic_analysis: dict) -> dict:
    ...
    if axes:
        ...
        if x_range and isinstance(x_range, list) and len(x_range) == 2 and x_range[0] is not None:
            x_range_dict = {"min": x_range[0], "max": x_range[1], "type": x_type}
        
        if y_range and isinstance(y_range, list) and len(y_range) == 2 and y_range[0] is not None:
