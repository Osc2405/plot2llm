...
def build_llm_context_section(semantic_analysis: dict) -> dict:
    ...
    if plot_types:
        if "line" in plot_types:
            hints_data["trend_analysis"] = "Look for trends, slopes, and inflection points."
            hints_data["direction_analysis"] = "Consider the overall direction and rate of change."
            hints_data["pattern_recognition"] = "Identify any patterns or cycles in the data."
        if "scatter" in plot_types:
            hints_data["cluster_analysis"] = "Check for clusters, outliers, and correlation between variables."
            hints_data["pattern_recognition"] = "Look for patterns in the distribution of points."
            hints_data["correlation_analysis"] = "Consider the strength and direction of any relationship."
        if "histogram" in plot_types:
            hints_data["shape_analysis"] = "Observe the distribution shape and spread."
            hints_data["peak_analysis"] = "Look for peaks, valleys, and overall symmetry."
            hints_data["statistical_analysis"] = "Consider the skewness and kurtosis of the distribution."
        if "bar" in plot_types:
            hints_data["categorical_comparison"] = "Compare the heights of the bars for categorical differences."
            hints_data["ranking_analysis"] = "Look for the largest and smallest categories."
            hints_data["distribution_analysis"] = "Consider the overall distribution of values across categories."
    
    # Si no se encontraron hints específicos, usar genéricos
    if not hints_data:
        hints_data["general_analysis"] = "Interpret the axes, labels, and data points to understand the visualization."
